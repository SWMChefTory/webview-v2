...
export function SearchRecipeMobile() {
  const controller = useSearchRecipeController("mobile");

  return (
    <div className="flex flex-col w-screen h-screen overflow-hidden">
      <Header
        leftContent={
          <HeaderLeftContentMobile
      ...
      {!controller.isSearching && (
        <div className="flex flex-col w-full h-full overflow-y-scroll">
          <DefaultContentMobile controller={controller} />
        </div>
      )}
    </div>
  );
...
const HeaderLeftContentMobile = memo(({
  router,
  keyboardInput,
  setKeyboardInput,
  inputRef,
...
HeaderLeftContentMobile.displayName = 'HeaderLeftContentMobile';

const AutoCompleteDropdownMobile = memo(({
  hasAutocompletes,
  autocompletes,
  keyboardInput,
  onSelect,
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {!hasAutocompletes ? (
            <DefaultContentMobile controller={controller} onSearchSelect={onSearchSelect} />
          ) : (
...
AutoCompleteDropdownMobile.displayName = 'AutoCompleteDropdownMobile';

const DefaultContentMobile = memo(({
  controller,
  onSearchSelect,
}: {
  controller: ReturnType<typeof useSearchRecipeController>;
...
DefaultContentMobile.displayName = 'DefaultContentMobile';

const CollapsedPopularSearchMobile = memo(({
  autocompletes,
  scrollContainerRef,
  onClick,
}: {
  ...
  return (
    ...
    >
      ...
      {autocompletes.map((item, index) => (
        <div
          key={index}
          className="flex items-center gap-2.5 px-3 absolute w-full h-[60px] cursor-pointer hover:bg-orange-50 transition-all duration-200 group rounded-lg"
...
CollapsedPopularSearchMobile.displayName = 'CollapsedPopularSearchMobile';
